...
const TablePage = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post("http://localhost:8080/api/teacher-positions", formData);
      console.log("Dữ liệu gửi đi:", response.data);
      setData([...data, response.data]); // Cập nhật danh sách với vị trí mới
      setShowModal(false); // Đóng modal sau khi tạo mới
      setFormData({
        code: "",
        name: "",
        description: "",
