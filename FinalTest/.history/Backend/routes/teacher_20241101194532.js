...
router.get('/teachers', async (req, res) => {
    try {
        const teachers = await Teacher.find({ isDeleted: false })
            .populate({
                path: 'userId',
                select: 'name email phoneNumber address -_id' // Chỉ lấy các trường cần thiết từ User
            })
            .populate({
                path: 'teacherPositionsId', // Đảm bảo tên trường đúng
                select: 'name code -_id' // Chỉ lấy tên và mã của vị trí
            })
            .select('code isActive degrees startDate'); // Chọn các trường cần thiết từ Teacher
